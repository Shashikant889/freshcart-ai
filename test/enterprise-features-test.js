...
async function runEnterpriseTests() {
  ...
  test('Calculate accurate macro breakdown (Calories, Protein, Carbs, Fiber, Fat)', () => {
    const cartItems = [
  ...
  test('Detect allergens (Lactose / Gluten) against user profile with alerts', () => {
    const cartItems = [
  ...
  test('Nutri-Score assigns Grade A/B for high fiber/protein organic baskets', () => {
    const healthyBasket = [
  ...
  test('Smart Substitution engine suggests allergy-safe & healthier alternatives', () => {
  ...
  test('Perishable markdown increases dynamically as days to expiry decrease', () => {
  ...
  test('Food waste prevention grams correctly estimated for catalog flash deals', () => {
  ...
  test('Optimize warehouse walk path starting & returning to Packing Station', () => {
  ...
  test('FreshWallet API provides balance and welcome credits', async () => {
  ...
  test('FreshWallet Top-Up increases balance and logs transaction', async () => {
  ...
  test('Split-payment calculates exact wallet deduction and remaining gateway balance', async () => {
  ...
  test('Create Group Buying Lobby & List active community lobbies', async () => {
  ...
  test('Joining group lobby dynamically upgrades community discount tier (5% -> 8% -> 10% -> 15%)', async () => {
    const createRes = await request('POST', '/api/group-orders/create', {}, {
  ...
  test('Supplier ROP math generates accurate Safety Stock and Reorder alerts', async () => {
    // Obtain admin token
  ...
  test('Supplier Warehouse Picker Route API responds with valid 2D TSP itinerary', async () => {
    const loginRes = await request('POST', '/api/auth/login', {}, { email: 'admin@freshcart.com', password: 'admin123' });
    const adminToken = loginRes.data.data.token;
